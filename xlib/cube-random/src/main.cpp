...
int main()
{
    ...
    while (!globalAbortFlag)
    {
        ...
        for(int i=0; i< 36; ++i)
        {
            colorBufferData[i*3] = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
            colorBufferData[i*3 + 1] = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
            colorBufferData[i*3 + 2] = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
        }
